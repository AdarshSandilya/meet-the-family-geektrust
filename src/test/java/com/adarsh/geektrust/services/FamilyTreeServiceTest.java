...
import com.adarsh.geektrust.strategies.MaternalUncleStrategy;
...
class FamilyTreeServiceTest {
    ...
    @Mock
    private FetchRelationStrategyFactory strategyFactory;
    @Mock
    ...
    @Test
    void getRelativesOf_should_call_proper_strategy_to_fetch_the_relatives() {
        String type = RelationshipType.SISTER_IN_LAW.getValue();
        MaternalUncleStrategy maternalUncleStrategy = mock(MaternalUncleStrategy.class);

        when(familyTreeRepository.findPersonByName(person.getName())).thenReturn(person);
        when(strategyFactory.getStrategy(RelationshipType.SISTER_IN_LAW)).thenReturn(maternalUncleStrategy);

        assertDoesNotThrow(() -> {
            service.getRelativesOf(person.getName(), type);
        });

        verify(maternalUncleStrategy, times(1)).apply(person);
    }
